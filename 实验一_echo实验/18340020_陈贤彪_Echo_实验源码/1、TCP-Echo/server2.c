     ...
     //fsin 客户端地址
        ...
        while(1){                                           	// 检测是否有按键,如果没有则进入循环体执行
          ...
          printf("\n正在接受请求");
           ...
           // 如果在连接请求队列中有连接请求，则接受连接请求并建立连接，返回该连接的套接字，否则，本语句被阻塞直到队列非空。fsin包含客户端IP地址和端口号
        ...
        // int aa= recv(ssock,rec, BUFLEN, 0);  
         ...
         //printf("收到消息:");
          printf("\n收到消息:  %s",rec);
           ...
           int len = strlen(pts);
           sprintf(sen,"%s %s",pts,rec);
              int cc = send(ssock, sen, strlen(sen), 0);   
         //int cc = send(ssock, pts, strlen(pts), 0);    
           //cc = send(ssock, pts, strlen(pts), 0);   	// 第二个参数指向发送缓冲区，第三个参数为要发送的字节数，第四个参数一般置0，返回值为实际发送的字节数，出错或对方关闭时返回SOCKET_ERROR。
          printf("\n收到时间:%s", pts);                                  	// 显示发送的字符串     
          close(ssock);                              		// 关闭连接套接字
     }
     close(msock);                                 		// 关闭监听套接字
